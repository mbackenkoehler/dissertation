\chapter{Truncations for Stationary Distributions}
...
In \cite{gupta2017finite} SSA estimates are used to set up an increasing family of truncations.
...
An approach where quantities are only described in terms of their magnitude has been proposed in \cite{ceska2019semi}. This allows for an efficient qualitative analysis of both dynamic and transient behavior.

An aggregation scheme similar to the one used here has been previously proposed in \cite{backenkohler2020analysis} to analyze the bridging problem on Markov population models.
...
Stationary distributions are connected to the \emph{long-run} behavior of an MPM \cite{dayar2011bounding}, as the system's distribution will converge to the (unique)
...
In this work, we propose a  method to identify a truncation that optimizes the trade-off between the size of the considered state-space and the approximation error due to the finite state-space projection.
...
\begin{figure}[t]
    ...
    \caption{The state-space refinement algorithm on a birth-death process. From left to right the state size is halved and states with low probability are removed from the truncation. The final truncation is a typical truncation with states of size 1 and the initial states are of size $2^4$.}
...
are typical benchmarks for the analysis of MPMs. For most of them, appropriate Lyapunov functions
...
\begin{figure}
	...
	\caption{The sizes of the final truncation vs.\ the threshold parameter $\epsilon$ (\autoref{model:bd} and \autoref{model:excl_switch}).}
...
\begin{figure}
    ...
    \caption{The approximate stationary distribution of the exclusive switch (\autoref{model:excl_switch}) obtained with $\epsilon=\text{1e-4}$.}
...
\begin{table}
    ...
    \toprule
     ...
     \midrule
         ...
         \bottomrule
    \end{tabular}
    \caption{Results for \autoref{model:excl_switch}: The characteristics of the lower-upper bound intervals on the conditional probability and the upper bound on mass not contained in the truncation are given.}
    \label{tab:intervals}
\end{table}
...
\begin{figure}
	...
	\caption{The approximate marginal distributions of the stationary distribution based on the truncation derived with $\epsilon=0.1$.}
\label{fig:p53:dist}
\end{figure}
