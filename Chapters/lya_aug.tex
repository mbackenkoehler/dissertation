...
The drift \eqref{eq:drift} plays a central role in this chapter.\marginpar{In contrast to page~\ref{eq:drift}, we make the dependency on the Lyapunov function $g$ explicit here.}
\begin{equation}
...
Clearly a positive linear factor $m$ to $g$ factors out, i.e.\ $Q(mg)(x)=mQg(x)$ for $m>0$.
Consequently, if the drift is scaled by its maximum value, the scaled version is invariant to linear
transforms of $g$:
\marginpar{A slight abuse of notation.}
\begin{equation}
	\frac{Q(mg)(x)}{\max_{x\in\mathcal{S}}Q(mg)(x)}
	=
...
In Figure~\ref{fig:truncation}, we contrast this result to the solution given by the choice of $g^*(x)=(x - 250)^2$, which gives a much tighter subset with the same guarantees.
...
\begin{figure}[htb]
    ...
    \caption[Manually augmented Foster-Lyapunov function]{\label{fig:truncation}Different example Lyapunov functions for Model~\ref{model:bd}. The drifts $d$, $d^*$, and $d'$ are scaled and the appropriate threshold for $\epsilon=0.1$ is given.}
\end{figure}
\end{example}
