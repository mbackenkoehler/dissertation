\chapter{State-Space Aggregation}\label{sec:statagg:aggregation}
...
\begin{figure}
	...
	\caption{A lumping approximation of \autoref{model:bd} on the state-space truncation to $[0, 200]$ on $t\in[0, 50]$. On the left-hand side solutions of a regular truncation approximation and a lumped truncation (macro-state size is 5) are given.}
    \label{fig:lumped}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[scale=.6]{gfx/lumped_dist.pdf}
    \caption{The terminal distributions $\Pr(X_{50}=x_i)$ of a lumped approximation and a truncation at original granularity are given.}
