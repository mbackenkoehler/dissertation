...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Splitting Algorithm}\label{sec:splitting}
...
\Output{estimate using linear control variates}
  $L\leftarrow\{\lambda_i\sim \pi_\lambda \mid 1\leq i< n_{\lambda} \} \cup \{ 0 \}$\label{line:resampling:lambda_sample}\tcp*[r]{initialization}
  $P \leftarrow \{ ({m}, \lambda) | 1\leq\lvert {m} \rvert \leq n_{\max}, \lambda\in L\}$\label{line:resampling:init_covs}\;
  $P_{\text{all}} = \emptyset$
    \For{$i=1,\dots,n_r$\label{line:resampling:main_loop} \tcp*[r]{resampling}}{
    ...
    $(\hat{\Sigma},\hat{V}, \hat{Z}) \leftarrow {\text{SSA}}_{\text{CV}}(\pi_0,T,P_{\text{all}},5d)$\label{line:resampling:search_loop}\tcp*[r]{covariance}
     $P^*=\emptyset$\;
     \While{$\exists i:
        (m_i, \lambda_i)\in P_{\text{all}}\setminus P^*\land$\\
        \hspace{4em}$\hat{\gamma}_{iv} \prod_{j=1;(m_j, \lambda_j)\notin P^*}^{\left|P_{\text{all}}\right|}
        \hat{\gamma}_{ij}^{-1}
        >\epsilon$\tcp*[r]{selection}\label{line:resampling:selection_loop}}{
    ...
    $(\hat{\Sigma},\hat{V}, \hat{Z}) \leftarrow \text{SSA}_{\text{CV}}(\pi_0,T,P^*,n)$\tcp*[r]{estimation}\label{line:resampling:sim}
...
\begin{model}[Lac operon]This is a well-known model of genetic regulation with positive feedback \parencite{stamatakis2009comparison}. Its reactions are
 \begin{gather*}
