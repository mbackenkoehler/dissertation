\chapter{Linear Control Variates for Monte~Carlo Estimation}\label{ch:cvinsrns}
...
\begin{algorithm}
    ...
    $\tau\leftarrow \text{append}(\tau, (s, t))$\;
        ...
        \For{$(m,\lambda)\in \mathit{keys}(A)$}{
            $A[(m,\lambda)]\leftarrow A[(m,\lambda)] + \left(e^{ \lambda(T - t - \delta)} - e^{ \lambda (T - t) }\right)/\lambda$\;
        }
        $s\leftarrow s + v_k$\;
        $t \leftarrow t + \delta$\;
    }
