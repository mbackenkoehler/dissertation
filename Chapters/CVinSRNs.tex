\chapter{Control Variates for Monte-Carlo Estimation}\label{ch:cvinsrns}
...
As in the previous chapter,\turnto{eq:exp_constraint} multiplication with $w(t)$ and integration on $[t_0, T]$ yields~\cite{dowdy2018dynamic,sakurai2019bounding}
...
\begin{align*}
	X + Y &\xrightarrow{.001} B + Y &
    B + Y &\xrightarrow{.001} 2 Y\\
	Y + X &\xrightarrow{.001} B + X &
...
\[
	X^X_t+X^Y_t+X^B_t=\mathrm{const.}$, $\forall t\geq 0\,,
\]
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=.6]{gfx/legend.pdf}\\
...
\begin{figure}[htb]
	...
	\includegraphics[scale=.35]{gfx/sample_size_efficiency.pdf}}\quad
	\subfloat[Slowdown v.\ sample size]
	{\label{fig:sample_sizes:slowdown}
	\includegraphics[scale=.35]{gfx/sample_size_slowdown.pdf}}
