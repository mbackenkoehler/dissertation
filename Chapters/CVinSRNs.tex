\chapter{Control Variates for Stochastic Simulation}
...
Analysis approaches based on sampling, such as the Stochastic Simulation Algorithm (SSA) \cite{gillespie1977exact}, can  be applied
...
The exact moment dynamics can be expressed as an infinite-dimensional system of ODEs, which cannot be directly  integrated for a transient analysis.
...
In this work, we propose a combination of such moment constraints with the SSA approach.
...
These constraints can be used  as  (linear) control variates in order to improve the final estimate and reduce its variance \cite{lavenberg1982statistical,szechtman2003control}.
...
The incorporation of control variates into the SSA
...
close the ODE system given by the moment equations.
...
MPM~\cite{anderson2018low}. In this techniques estimates are combined
...
computing the sum
% \vw{ hier steht erstmal kein Integral, sondern die Zeitableitung von pi} 
...
The variable $Z$ is called a \emph{control variate}.
If a control variate $Z$ is correlated with $V$, we can
use it to
...
since it represents the only factor incurring a direct cost increase in the SSA itself (\autoref{line:run_ssa}).
...
\begin{figure}
    ...
    \end{minipage}
    \caption{Different decision functions
...
\begin{figure}
    ...
    \end{minipage}
	\caption{(a, b) The absolute correlation of different constraints {to $V$}
...
\begin{figure}[t]
    ...
    \centering(b)
    \end{minipage}
    \caption{(a) The effect of including control variates (CV) on the mean estimates
    $\hat{\mathbb{E}}(X^M_2)$ in the dimerization case study. Parameters
...
\begin{figure}[t]
    ...
    \end{minipage}
    \caption{The efficiency for different redundancy policies $\phi$ and maximal 
...
\begin{figure}[tb]
    ...
    \end{minipage}
    \hfill
    \caption{A visualisation of the trade-off between variance reduction $\sigma_0^2/\sigma_1^2$
...
\begin{figure}[h]
    ...
    \end{minipage}
    \caption{The empirical efficiencies for different $n_\lambda$ and $k_{\min} $.
...
\begin{figure}
	...
	\includegraphics[scale=.4]{gfx/sample_size_efficiency.pdf}}\quad
	\subfloat[Slowdown v.\ sample size]
	{\label{fig:sample_sizes:slowdown}
	\includegraphics[scale=.4]{gfx/sample_size_slowdown.pdf}}
...
the supplementing a standard SSA estimation with moment information
