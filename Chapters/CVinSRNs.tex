...
The time-evolution of the expected value $\E{f(X_t)}$ of some function $f$ is given by \eqref{eq:mom_ode}\turnto{eq:mom_ode}.
  ...
  $P_{\text{all}} = \emptyset$
    ...
    $(\hat{\Sigma},\hat{V}, \hat{Z}) \leftarrow {\text{SSA}}_{\text{CV}}(\pi_0,T,P_{\text{all}},5d)$\label{line:resampling:search_loop}\tcc*[r]{covariance}
     ...
     \While{$\exists i:
        ...
        \hspace{4em}$\hat{\gamma}_{iv} \prod_{j=1;(m_j, \lambda_j)\in P^*}^{\left|P_{\text{all}}\right|}
        \hat{\gamma}_{ij}^{-1}
...
This selection is done, until some lower threshold $\epsilon$\marginpar{Here, $\epsilon=0.1$.} is reached (\autoref{line:resampling:selection_loop}).
