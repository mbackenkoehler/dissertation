...
%Markov Chains}
%\titlerunning{Bounding Mean First Passage Times in MPMs}
...
Popular exact methods for CMTCs rely on numerical approaches that explicitly consider each system state individually.
...
distribution~\cite{hespanha2008moment}. %\vw{Hespanha macht doch lognormal?}\todo{in dem papier macht er eine survey f\"ur sein tool}
...
Here, we put forward a method based solely on moments that gives \emph{exact bounds}  for Mean First Passage Times (MFPTs) and reachability probabilities in MPMs. For a set of states, the MFPT within a fixed
...
% For instance, in one of our case studies  $100,\!000$ SSA runs are necessary to
% achieve a relative   width of 0.9\% for the MFPT confidence interval.
% The SDP solver, however, returns a guaranteed interval with 
...
%Here, the approach by Lasserre is adapted to MPMs.
...
the CME \eqref{eq:cme} becomes
\begin{align*}
\frac{d}{dt}\pi((x^{(M)}, x^{(D)}), t)
=&\lambda \pi((x^{(M)}-1, x^{(D)}), t)\\ &+ \frac{\delta}{2} \, (x^{(M)}+2) (x^{(M)} +
  1) \pi((x^{(M)}+2, x^{(D)}-1), t)\\ &- (\lambda + \frac{\delta}{2} \, x^{(M)} (x^{(M)} -
  1))\pi((x^{(M)}, x^{(D)}), t)\,.
\end{align*}
\fi
...
In this work, we are interested in \emph{first passage times} of such processes.
...
%Note, that moments can also be defined for non-probability measures, i.e.\ for
%measures with total measure $\neq 1$.
...
Therefore we can view \eqref{eq:constraint} as the description of a relationship between
...
\begin{figure}
    \myfloatalign
    \subfloat[FPT distribution.]
    {\label{fig:dim_fpt:fpt}
	\includegraphics[scale=.8]{gfx/fpt_dist_dim.pdf}} \quad
    \subfloat[MFPT bounds.]
    {\label{fig:dim_fpt:mfpt}%
        \includegraphics[scale=.8]{gfx/fpt_bounds_dim.pdf}} \\
...
%     \end{minipage}
% 	\caption{    (a) The distribution of $\tau$ estimated based on 100,000 SSA samples.
%     (b) The bounds based on the SDP in~\eqref{eq:sdp_for_fpt} with different moment
...
\begin{figure}
    \myfloatalign
    \subfloat[MFPT bounds over $T$.]
    {\label{fig:dim_fpt_fin:mfpt}
	\includegraphics[scale=.8]{gfx/mfpt_bounds.pdf}} \quad
    \subfloat[Reaching probabilities up to $T$.]
    {\label{fig:dim_fpt_fin:rp}
        \includegraphics[scale=.8]{gfx/reachability_probs.pdf}}
...
%     The results for SDP relaxations of orders 1 (light) to 6 (dark) are shown.
%     (a) The bounds on the MFPT for differing time-horizons $T$.
...
\begin{model}[Parallel independent dimerizations]\label{model:double_dim}
...
% Still, the possible state-space reaches a size of $200^2=40,\!000$.
...
The SSA   (using $n=10,\!000$ runs) gives the estimate $\E{\tau}\approx 0.028378$ %\pm 1.3025e-04$ (99\%-CI)
...
\subsection{Hybrid Models and Multi-Modal Behavior}
...
% We can use this fact two ways: First we could use the same moment
% constraints
