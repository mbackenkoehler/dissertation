...
usually generated using the \ac{SSA}~\cite{gillespie1977exact}.
...
probabilities and \aclp{MFPT}~\cite{hayden2012fluid,bortolussi2013model,bortolussi2014stochastic}.
...
Here, we put forward a method based solely on moments that gives \emph{exact bounds}  for \acfp{MFPT} and reachability probabilities in \acp{MPM}. For a set of states, the MFPT within a fixed
...
Considerable effort has been directed at the analysis of \acl{FPT}
...
Specifically,\marginpar{More details on the derivation of the moment \acp{ODE} is given in \autoref{sec:moments_bg}.}
...
The empirical \ac{FPT} distribution based on 100,000 \ac{SSA} simulations is given in \autoref{fig:dim_fpt:fpt}
...
%     The results for \ac{SDP} relaxations of orders 1 (light) to 6 (dark) are shown.
...
The \ac{SSA}   (using $n=10,\!000$ runs) gives the estimate $\E{\tau}\approx 0.028378$ %\pm 1.3025e-04$ (99\%-CI)
