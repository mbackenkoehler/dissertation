...
\begin{table}[t]
\centering
    \caption[\ac{SDP} \ac{MFPT} bounds]{\ac{MFPT} bounds on \autoref{model:dim} for $X_t \geq 5$ and $T=1$, \autoref{model:double_dim} for $X_t^{(M_1)}\geq 200$ or $X_t^{(M_2)}\geq 200$ and $T=10$, and \autoref{model:gexpr} for $X_t^{(P)}\geq 5$ and $T=10$.\label{tab:bounds}}
