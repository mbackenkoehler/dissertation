...
\acs{CSL}~\parencite{aziz1996verifying,baier2000model,baier2003model},
...
\subsection{Hausdorff Constraints}
...
\begin{equation}\label{eq:hausdorff_constraint}
	\int_{{[0,H_1]\times\dotsm\times [0,H_n]}}\prod_{i = 0}^n x_i^{\ell_i}{(H_i-x_i)}^{k_i}\,d\mu{(\vec{x}})\geq 0
\end{equation}
...
\[
    (y + x)^k = \sum_{j=0}^k \binom{k}{j}y^{j}x^{k-j}\,.
\]
That is, given $n$-dimensional vectors $k$, $x$, and $y$
\begin{multline*}
    (y_1+x_1)^{k_1}\dotsm(y_n+x_n)^{k_n} =\\ \sum_{j_1=0}^{k_1}\dotsm\sum_{j_n=0}^{k_n} \binom{k_1}{j_1} y_1^{j_1}x_1^{k_1-j_1} \dotsc \binom{k_n}{j_n} y_n^{j_n}x_n^{k_n-j_n}\,.
\end{multline*}
% In multi-index notation
% \[
%     (y + x)^k = \sum_{\nu\leq k} \binom{k}{\nu} y^{\nu}x^{k-\nu}\,.
% \]
Thus, \eqref{eq:hausdorff_constraint} becomes
\begin{multline}
    \sum_{j_1=0}^{k_1}\dotsm\sum_{j_n=0}^{k_n} \binom{k_1}{j_1} H_1^{j_1}(-1)^{k_1-j_1}x_1^{k_1-j_1+\ell_1} \dotsc \\
    \binom{k_n}{j_n} H_n^{j_n}(-1)^{k_n-j_n}x_n^{k_n-j_n+\ell_n}\geq 0\,.
\end{multline}
...
such as CVXPY~\parencite{cvxpy}. We therefore only give the \ac{SDP} in the more intuitive format.
This problem can be solved using off-the-shelf \ac{SDP} solvers such as MOSEK~\parencite{mosek},
CVXOPT~\parencite{vandenberghe2010cvxopt}, or SCS~\parencite{scs}.
...
the time-horizon, and adjust the scaling accordingly (suggested in \parencite{dowdy2018dynamic}).
...
We implemented and solved the \ac{SDP} programs described above using optimization suite MOSEK~\parencite{mosek} (version 9.1.2) via the CVXPY
interface~\parencite{cvxpy} (version 1.0.24).
...
switching time distribution is of particular interest~\parencite{spieler2011model,barzel2008calculation}.
