...
\begin{equation}\label{eq:hausdorff}
	\int_0^1 x^n(1-x)^k\,d\mu(x) \geq 0, \quad \forall n, k\in\mathbb{N}\,.
\end{equation}
The validity of this condition is easy to see since
\begin{equation}\label{eq:hausdorff_integrand}
	x^n(1-x)^k\geq 0, \quad \forall x\in[0,1]\,.
\end{equation}
...
\begin{example}
	Let $n = 2$, $k=2$. Then the \eqref{eq:hausdorff} becomes
	\[
...
\begin{equation}\label{eq:hausdorff_scaled}
	\int_0^H x^n(H-x)^k\,d\mu(x) \geq 0, \quad \forall n, k\in\mathbb{N}
\end{equation}
...
For $n$ dimensions
\begin{equation*}
	\int_{{[0,1]}^n}\prod_{i = 0}^n x^{n_i}{(1-x)}^{k_i}\,d\mu{(\vec{x}})\geq 0
\end{equation*}
for all $n_i, k_i\in\mathbb{N}$ and $i\in \{0,\dotsc, n\}$.
With arbitrary positive upper bounds $H_1, H_2, \dotsc, H_n$ this becomes
\begin{equation}
	\int_{{[0,H_1]\times\dotsm\times [0,H_n]}}\prod_{i = 0}^n x^{n_i}{(H_i-x)}^{k_i}\,d\mu{(\vec{x}})\geq 0
\end{equation}
