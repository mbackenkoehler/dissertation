...
A popular approach is found in \acsfont{dwSSA} \parencite{kuwahara2008efficient,daigle2011automated}.
...
If the complete backward probabilities are available, we would have the optimal biasing.
\marginpar{If the backward probabilities are known, the event probability is known.}
...
In \autoref{fig:biases}, we illustrate this scheme for a birth-death process (\autoref{model:bd}) with goal macro-state $[24,31]$ at $T=10$ and $X_0=0$.
\begin{figure}
    \centering
...
\marginpar{In fact, such an approach might be better suited in the example above, where we aim to push the process to a specific micro-state.}
...
\SetKwInOut{Output}{output}
    ...
    \Loop{\label{line:outer_loop}}{
        ...
        \While{$X>1 - \exp(-a_0 \delta-\Delta)$\tcc*[r]{find interval}\label{line:tloop}}{
            ...
            $a_0\leftarrow\sum_j\hat{\tilde{\alpha}}_{j}(s)$\label{line:jump_end}\tcc*[r]{exit rate}
        }
        ...
        $k\leftarrow$ sample $j$ with probability $\hat{\tilde{\alpha}}_{j}(s)/\sum_i\hat{\tilde{\alpha}}_{i}(s)$\label{line:sample_dyn_r}\;
        ${\ell}_{1} \leftarrow \hat\tilde{\alpha}_{k}(s) \exp\left(-\Delta - \delta a_0\right)$\;
...
%\begin{figure}[htb]
    ...
    %\caption[Dynamic biasing for the Poisson process]{\label{fig:poisson_rare}The dynamic biasing of a Poisson process with rate 10 based on a aggregation of \num{10} states.}
%\end{figure}

%\begin{figure}[htb]
    %\centering
    %\includegraphics[width=\textwidth]{gfx/bd_is.pdf}
    %\caption[Dynamic biasing for the Poisson process]{\label{fig:bd_rare}The dynamic biasing of a birth-death process aggregation of \num{10} states and the target event $X(10)=30$.}
%\end{figure}
...
\marginpar{This approximation is different from the previous version on page~\pageref{page:approx}, because we previously had $k=1$.}
\[
\begin{split}
...
\acsfont{dwSSA} \parencite{daigle2011automated} essentially derives a constant bias coefficient for each reaction.
...
This case study illustrates how the finite sample behavior can significantly deviate from a normal distribution.
The estimate distribution tends to be asymmetrical with a most samples being below the true mean, while few samples are larger.
