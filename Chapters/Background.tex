...
A \ac{CTMC} is a Markov process, that takes discrete values $\mathcal{S}=\{s_0, s_1,\dots\}$ over continuous time $T=[0,\infty)$.\marginpar{The time-discrete analogue is the \acs{DTMC}.}
If further
...
In particular, \acp{ODE} for the expected values of an \ac{MPM} can be derived using its generator.\marginpar{This is called the \emph{drift} (see \autoref{sec:statagg:lyapunov}).}
