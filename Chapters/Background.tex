...
It is well-known that for a \ac{CTMC} $X$, ergodicity can be proven by a Foster-Lyapunov\marginpar{This is the stochastic analogue of the Lyapunov function for \acp{ODE}.} function $g:\mathcal{S}\to\mathbb{R}_+$ \cite{meyn1993stability,dayar2011bounding}.\footnote{The positivity requirement can somewhat be relaxed. The function only needs to be bounded from below since it can be shifted to the positives. In practice this shift can be ignored since it cancels out in the drift \eqref{eq:drift}.}
...
\begin{align}
	...
	&Qg(x)< \infty,\; \forall x\in C\,, \text{ and}\\
