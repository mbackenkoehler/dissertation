...
An \acf{MPM}
\marginpar{Other names for this model class are \acf{pCTMC}, \acf{CRN}, and \acf{SRN}.}
...
We can apply $Q$ to $f$ --~a polynomial function~-- such that\marginpar{This is called the \emph{drift} (see \autoref{sec:statagg:lyapunov}).}
...
\end{equation}
As\marginpar{Note that we end up with \eqref{eq:mom_ode} taking the expectation.} such the drift can be interpreted as the expected local tendency of change of a scalar valued function $g$, i.e.\ 
\[
